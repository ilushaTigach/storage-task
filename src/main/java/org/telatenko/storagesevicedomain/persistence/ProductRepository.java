...
public interface ProductRepository extends JpaRepository<ProductEntity, UUID> {
    ...
    @Query(value = "SELECT p.* FROM (SELECT *, ROW_NUMBER() OVER (ORDER BY id) AS rn FROM products) p WHERE p.rn BETWEEN :startId AND :endId", nativeQuery = true)
    List<ProductEntity> findAllInRange(@Param("startId") long startId, @Param("endId") long endId);
}
